...
//help us convert the SVG used for our NFT image and the JSON for its metadata to Base64 in Solidity.
import { Base64 } from "./libraries/Base64.sol";

import 'hardhat/console.sol';

contract Domains is ERC721URIStorage{
    //help us keep track of tokenIds by OpenZeppelin
    using Counters for Counters.Counter;
    //_tokenIds is an object that lets us access and set our NFT's unique token number.
    Counters.Counter private _tokenIds;

    //Here's our domain TLD
    string public tld;

    //store our NFT images on chain as SVGs
    ...
    //store their name to address
    mapping(string => address) public domains;
    mapping(string => string) public records;

    //ERC721(the NFT Collection's Name, the NFT's Symbol)
    ...
    function register(string calldata name) public payable  {
        ...
        //create the JSON metadata of our NFT by combining strings and encoding as base64.
        //NFTs use JSON to store details like the name, description, attributes and the media.
        //combine strings with abi.encodePacked to make a JSON object, then encode it as Base64 string before setting it as the token URI.
        ...
        //Set the NFTs data -- in this case the JSON blob with our domain's info
    ...
    function setRecord(string calldata name, string calldata record) public {
        //check that the only owner can record data to its name
        require(domains[name]==msg.sender);
        records[name]= record;
    }
